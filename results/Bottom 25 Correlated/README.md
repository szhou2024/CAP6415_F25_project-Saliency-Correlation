...
Examples below:
![1st Lowest](./images/01.png)

![15th Lowest](./images/15.png)

![23rd Lowest](./images/23.png)
...
Reshaping all 1280 channels to a singular dimension introduces another challenge. If the dimensions are reshaped to lowest spatial size by default (pool layer 8x10), this is akin to **heavily** blurring finer spatial layers like layer 1's 120x160, as well as the other intermediate layers. This likely causes too much information loss in learned feature maps of larger spatial dimensions, and the expected result is that textures and other detailed patterns become too distorted to activate. Images with high texture detail, especially food objects, likely exhibit misalignment due to this downsampling.

The advantage of using 8x10 dimensions is greater computational and memory efficiency. On the other hand, as mentioned earlier, it is impossible to upsample all layers to 120x160, nor does it make sense because no new information is gained (would simply hook FPN layer 1 instead). A better compromise is layer 3 and/or 4, which have higher spatial dimensions, but still allow EigenCAM to run reasonably fast. I do this in the Deep Dive on Bottom 25 section. 

Examples below:
![2nd Lowest](./images/02.png)

![9th Lowest](./images/09.png)

![25th Lowest](./images/25.png)
